...
$database = (string) getenv('ERP_CUTTING_CONFIGURATION_RACE_DATABASE');
...
$target = static fn (string $sql): bool => str_starts_with(strtolower($sql), 'select ')
    && str_contains($sql, '`erp_custom_configurations`')
    && str_contains($sql, '`configuration_no`')
...
try {
    $result = $app->make(CuttingConfigurationService::class)->createNextVersion(
        (int) $args['id'], $args['payload'], (object) $args['user'], $args['permissions'], true,
    );
