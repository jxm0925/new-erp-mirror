...
try {
    if ($mode === 'setup') {
        ...
        $output = Item::create(['item_code' => $prefix.'-FG', 'item_name' => 'WO04 并发成品', 'item_type' => 'finished_good', 'unit_id' => $unit->id, 'is_stock_item' => true, 'is_production_item' => true, 'production_execution_mode' => 'unit', 'status' => 'enabled']);
        ...
        $routingOperationId = DB::table('erp_production_routing_operations')->insertGetId(['routing_id' => $routingId, 'operation_id' => $operationId, 'sequence' => 10, 'is_key_operation' => true, 'created_at' => now(), 'updated_at' => now()]);
        $order = SalesOrder::create(['sales_order_no' => $prefix.'-SO', 'customer_name' => 'WO04 并发客户', 'order_status' => 'confirmed', 'confirm_status' => 'confirmed', 'production_confirm_status' => 'confirmed', 'sales_user_legacy_id' => $ownerId, 'created_by_legacy_id' => $ownerId, 'total_amount' => 0, 'final_receivable_amount' => 0, 'funding_policy_snapshot' => ['policy_type' => 'full_prepay', 'shipment_requires_full_payment' => true], 'required_delivery_date' => '2026-09-20']);
    ...
    if ($mode === 'cleanup') {
        ...
        $masterOrderIds = DB::table('erp_production_master_orders')->whereIn('sales_order_id', $orderIds)->pluck('id');
        ...
        $taskIds = DB::table('erp_production_tasks')->whereIn('work_order_id', $workOrderIds)->pluck('id');
        DB::table('erp_production_execution_events')->whereIn('aggregate_id', $taskIds)->where('aggregate_type', 'production_task')->delete();
        DB::table('erp_production_execution_commands')->where('initiated_by_legacy_id', $ownerId)->delete();
        DB::table('erp_production_target_material_requirements')->whereIn('work_order_id', $workOrderIds)->delete();
        DB::table('erp_work_order_material_supply_rules')->whereIn('work_order_id', $workOrderIds)->delete();
        DB::table('erp_production_labor_sessions')->whereIn('task_id', $taskIds)->delete();
        DB::table('erp_production_task_collaborators')->whereIn('task_id', $taskIds)->delete();
        DB::table('erp_production_task_targets')->whereIn('task_id', $taskIds)->delete();
        DB::table('erp_production_tasks')->whereIn('id', $taskIds)->delete();
        DB::table('erp_production_quantity_operations')->whereIn('work_order_id', $workOrderIds)->delete();
        $unitIds = DB::table('erp_production_units')->whereIn('work_order_id', $workOrderIds)->pluck('id');
        DB::table('erp_production_unit_operations')->whereIn('production_unit_id', $unitIds)->delete();
        DB::table('erp_production_units')->whereIn('id', $unitIds)->delete();
        $preparationOrderIds = DB::table('erp_production_preparation_orders')->whereIn('production_master_order_id', $masterOrderIds)->pluck('id');
        DB::table('erp_production_preparation_order_lines')->whereIn('preparation_order_id', $preparationOrderIds)->delete();
        DB::table('erp_production_preparation_orders')->whereIn('id', $preparationOrderIds)->delete();
        ...
        DB::table('erp_production_master_orders')->whereIn('id', $masterOrderIds)->delete();
        DB::table('erp_routing_operation_material_supply_rules')->whereIn('routing_operation_id', DB::table('erp_production_routing_operations')->whereIn('routing_id', $routingIds)->pluck('id'))->delete();
