...
function mount(production, cutting = { listTasks: async () => ({ data: [], meta: { total: 0 } }) }) {
  let page;
  const navigations = [];
  const source = fs.readFileSync(path.join(__dirname, '../pages/production/workbench/index.js'), 'utf8');
  vm.runInNewContext(source, {
    Page: value => { page = value; },
    require: moduleName => moduleName.includes('services/production') ? production : (moduleName.includes('services/cutting') ? cutting : { BadgePopup() {} }),
    wx: {
      getStorageSync: key => {
        if (key === 'erp_token') return 'real-token';
        if (key === 'erp_permissions') return ['production.task.view', 'production.work_order.view', 'production.material_picking.view', 'production.cutting.view'];
        return '';
      },
...
test('workbench renders real overview, four approved entries, and the cutting entry', async () => {
  const calls = [];
  const page = mount({
  ...
  }, { listTasks: async query => { calls.push(['cutting', query]); return { data: [], meta: { total: 3 } }; } });
  ...
  assert.deepEqual(Array.from(page.data.shortcuts, item => item.count), [6, 2, 5, 20, 3]);
  assert.equal(calls.every(([, query]) => query.page === 1 && query.per_page === 1), true);
  assert.equal(calls.find(([name]) => name === 'cutting')[1].scope, 'mine');
  assert.equal(calls.find(([name]) => name === 'cutting')[1].status_group, 'active');
  ...
  page.openShortcut({ currentTarget: { dataset: { key: 'cutting' } } });
  assert.deepEqual(page.__navigations.map(item => item.url), [
    ...
    '/pages/production/cutting-tasks/index'
  ]);
});
...
test('workbench source preserves approved responsive two-by-two structure', () => {
  ...
  assert.match(wxss, /grid-column:\s*1 \/ -1/);
