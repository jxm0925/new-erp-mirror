...
function mount(production = {}) {
  ...
  const storage = { erp_token: 'valid_mock_token', erp_user_name: '李师傅', erp_team_name: '电子装配一组' };
...
test('my-tasks loads tasks, computes progress, and formats status tags and CTA buttons correctly', async () => {
  ...
  assert.equal(item1.workerDesc, '李师傅 (责任人) · 协同 2人');
  ...
  assert.equal(item2.statusTagText, '待齐套 (需领料)');
  assert.equal(item2.kittingText, '常备料待盘点 (缺)');
  assert.equal(item2.kittingClass, 'col-warning');
  assert.equal(item2.ctaText, '物料核对 ➔');
  assert.equal(item2.ctaClass, 'cta-warning');
  assert.equal(item2.workerDesc, '李师傅 (独立作业)');
});
